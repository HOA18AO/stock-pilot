...
export default function PurchaseFormModal({ isOpen, onClose, onSuccess }: PurchaseFormModalProps) {
    ...
    const [mergedPreviewItems, setMergedPreviewItems] = useState<LineItem[]>([]);
    const [mergedPreviewTotals, setMergedPreviewTotals] = useState<Totals>({
        originalAmount: 0,
        totalFees: 0,
        totalTax: 0,
        finalAmount: 0,
    });
    const [mergedLinesCount, setMergedLinesCount] = useState(0);
    ...
    const resetForm = () => {
        ...
        setDescription('');
        setLineItems([]);
        setMergedPreviewItems([]);
        setMergedPreviewTotals({
            originalAmount: 0,
            totalFees: 0,
            totalTax: 0,
            finalAmount: 0,
        });
        setMergedLinesCount(0);
    ...
    const getLineItemMergeKey = (item: LineItem): string => {
        return `${item.productCode}|${item.unitCost}|${item.additionalFee}|${item.tax}|${item.notes}`;
    };

    const mergeDuplicateLineItems = (items: LineItem[]): LineItem[] => {
        const mergedMap = new Map<string, LineItem>();

        for (const item of items) {
            const key = getLineItemMergeKey(item);
    ...
    const calculateTotals = (items: LineItem[]): Totals => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const mergedItems = mergeDuplicateLineItems(lineItems);
        const totals = calculateTotals(mergedItems);

        setMergedPreviewItems(mergedItems);
        setMergedPreviewTotals(totals);
        setMergedLinesCount(lineItems.length - mergedItems.length);
    ...
    const handleConfirmCreate = async () => {
        ...
        try {
            const purchaseDetails = mergedPreviewItems.map(item => {
                const baseAmount = item.unitCost * item.quantity;
                const taxAmount = baseAmount * (item.tax / 100);
            ...
            const payload = {
                type: 'purchase',
                vendorCode,
                originalAmount: mergedPreviewTotals.originalAmount,
                additionalFee: mergedPreviewTotals.totalFees,
                tax: mergedPreviewTotals.totalTax,
                finalAmount: mergedPreviewTotals.finalAmount,
                status,
    ...
    return (
        <>
            ...
            {showConfirmation && (
                <OverlayBackdrop className="z-[60] flex items-center justify-center p-4">
                    <div className="bg-gray-800 rounded-xl max-w-3xl w-full border border-gray-700 overflow-hidden">
                        <div className="flex items-center justify-between p-5 border-b border-gray-700">
                            <h3 className="text-xl font-semibold text-white">Confirm Purchase Order</h3>
                            <button
                        ...
                        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                            <div className="text-sm text-gray-300">
                                Review the final order before submission. Duplicate lines are merged only when Product, Unit Cost, Additional Fee, Tax, and Notes are all identical.
                            </div>

                            {mergedLinesCount > 0 && (
                                <div className="bg-blue-900/30 border border-blue-700 text-blue-200 px-4 py-2 rounded-lg text-sm">
                                    {mergedLinesCount} duplicate line{mergedLinesCount > 1 ? 's were' : ' was'} merged in this final summary.
                                </div>
                            )}
                        ...
                        <div className="flex items-center justify-end gap-3 p-5 border-t border-gray-700 bg-gray-800">
                            <Button
                                variant="ghost"
                                type="button"
                                onClick={() => setShowConfirmation(false)}
